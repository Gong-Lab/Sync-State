function [sigWindows, thr, popCountSmooth, edges] = findPopulationEvents(eventTimes, binSize, winSize, nShuffle, alpha, doPlot)
...
%
% INPUTS:
%   eventTimes : cell array {N x 1}, each cell = vector of event times
...
eventTimes = cellfun(@(x) x(:)', eventTimes, 'UniformOutput', false);
...
%% Shuffle test (circular time shift per cell)
...
for s = 1:nShuffle
    ...
    for i = 1:numel(eventTimes)
        shift = rand() * dur;
        shifted = mod(eventTimes{i} + shift - tmin, dur) + tmin;
        shCounts(i,:) = histcounts(shifted, edges);
    end
...
if doPlot
    ...
    title('Significant population events');
